...
export class Conflict {
   ...
   * separatorMarker [TextEditorMarker]
   */
  constructor (prevConflict, ours, theirs, base, merge, separatorMarker) {

    // Join the linked list.
    this.prevConflict = prevConflict || null
    if (prevConflict) {
      this.nextConflict = prevConflict.nextConflict
      prevConflict.nextConflict = this
    } else {
  ...
  resolveAs (side) {
    if (this.prevConflict) {
      this.prevConflict.nextConflict = this.nextConflict
      this.prevConflict = null
    }
    if (this.nextConflict) {
      this.nextConflict.prevConflict = this.prevConflict
      this.nextConflict = null
    }
   ...
   */
  markers () {
    const ms = [this.ours.markers(), this.theirs.markers(), this.separatorMarker]
    if (this.baseSide) {
  ...
  static all (merge, editor) {
    ...
    editor.getBuffer().scan(CONFLICT_START_REGEX, (m) => {
      ...
      try {
        ...
        const prevConflict = conflicts[conflicts.length - 1];
        const newConflict = visitor.conflict(prevConflict);
...
class ConflictVisitor {
  ...
  visitSeparator (sepRowStart, sepRowEnd) {
    ...
    this.separatorMarker = marker
    this.previousSide = {
      marker: marker,
      followingMarker: null,
      refBannerMarker: null,
    }
  }
